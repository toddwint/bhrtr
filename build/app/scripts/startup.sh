...
if [ -e /opt/"$APPNAME"/scripts/.firstrun ]; then
    ...
    cp /opt/"$APPNAME"/scripts/ip-addrs-and-routes-add /usr/local/bin
    chmod 755 /usr/local/bin/ipcalc.py
    chmod 755 /usr/local/bin/ip-addrs-and-routes-add
fi
...
then
    ...
    cp /opt/"$APPNAME"/configs/bkhl.csv /opt/"$APPNAME"/upload
...
ip-addrs-and-routes-add >> /opt/"$APPNAME"/logs/"$APPNAME".log
